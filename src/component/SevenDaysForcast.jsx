    ...
    return(
        <>
           ...
           <div className="bg-[#202b3b] w-[90%] ml-auto mr-auto rounded-3xl shadow-md p-5 ">
               <h1 className={`uppercase font-mono font-semibold w-[90%] ml-auto mr-auto py-2 text-[#9399a2]  ${!isVisible? "text-md" : 'text-xl' }`}>7 day's forcast</h1>
               <div>
                  <ul className="flex flex-col gap-4">
                     {
                      newForecast.map((day , index)=>(
                        <li key={index} className={`flex items-center mr-auto ml-auto justify-between ${!isVisible? 'py-[0.07rem] w-4/5': 'py-[0.55rem] w-[90%]'}  ${index < 5  ? 'border-b-[1px] border-[#9399a2]' : ''}`}>
                            <p className={`font-rubik capitalize  text-[#9399a2] ${!isVisible? "text-lg" : 'text-xl' }`}>{days[new Date(day.time).getDay()]}</p>
                            <img src={imageMap[day.values.weatherCodeMax]} alt="icon" className={` ${!isVisible? "w-10 h-10" : 'w-16 h-16' }`} />
                            <p> <span className="font-rubik font-bold text-md text-white">{day.values.temperatureMax}</span> <span className="text-[#9399a2]">/</span><span className="text-sm font-mono font-normal text-[#9399a2]">{day.values.temperatureMin}</span></p>
                        </li>
                      ))
                     }
                  </ul>
